...
void main() {
	...
	vec3 godrayTint = mix(vec3(0.5, 0.6, 1.0), vec3(1.0, 0.85, 0.6), step(0.0, dot(normalize(shadowLightPosition), vec3(0.0, 1.0, 0.0))));
	color.rgb += godrays * godrayTint * 0.18;
	...
	vec3 outlineColor = color.rgb * 3.5;
	color.rgb = mix(color.rgb, outlineColor, outlineAmount * 0.5);
