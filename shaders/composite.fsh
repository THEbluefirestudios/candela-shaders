...
vec3 getShadow(vec3 shadowScreenPos){
	const float COLORED_SHADOW_SATURATION = 1.8;
	const int PCF_RADIUS = 2;
	const float PCF_SPREAD = 0.75;

	vec2 texelSize = PCF_SPREAD / vec2(shadowMapResolution);
	...
	for (int x = -PCF_RADIUS; x <= PCF_RADIUS; x++){
		for (int y = -PCF_RADIUS; y <= PCF_RADIUS; y++){
			...
			tintedShadow = mix(vec3(dot(tintedShadow, vec3(0.299, 0.587, 0.114))), tintedShadow, COLORED_SHADOW_SATURATION);
